...
if(mysqli_num_rows($query_describe) > 0){
    ...
    while($hasil_describe = mysqli_fetch_array($query_describe)){
        ...
        if($hasil_describe['Field'] != "timestamp" && strtolower($hasil_describe['Type']) != "text" && strtolower(substr($hasil_describe['Field'], 0, strlen("google_drive_pdf"))) != "google_drive_pdf"){
...
class get_'.$rest_name_.' extends CI_Model {
    ...
    function get_data(){
        ...
        $sql = "select '.$select_column.' from '.$nama_tabel.'".$clouse." order by id asc limit $iDisplayStart , $iDisplayLength";
