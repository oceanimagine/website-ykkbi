        ...
        <div class="col-md-12 col-sm-12 col-xs-12" style="padding-right: 0px; padding-left: 0px;">
            <div style=" width: 100%; height: 100%; background-color: #f1f1f1; padding: 0.01em 16px; border-radius: 0px; margin-left: auto; margin-right: auto; padding-top: 15px; padding-bottom: 15px; box-shadow: 0 4px 14.48px rgb(0 0 0 / 25%); z-index: 99; position: relative;">
                <div class="header_article" style="background-color: #3b3c8c; width: 100%; padding: 8px 16px; border-radius: 4px;">
                    <h3 style='color: white; margin-bottom: 2px; text-align: center;'>Berita Video</h3>
                </div>
                <div class="row">
                        ...
                        <section id="article" style="background-color: #f1f1f1; padding-bottom: 0px; padding-top: 16px; position: relative; z-index: 99;">
                            <div class="row">
                                ...
                                if(isset($_GET['search']) && $_GET['search'] != ""){
                                    ...
                                    $where_search = " where judul_video like '%" . strtolower(str_replace(array_reverse($GLOBALS['replace']),array_reverse($GLOBALS['search']), strip_tags($search))) . "%' or deskripsi_singkat like '%" . strtolower(str_replace(array_reverse($GLOBALS['replace']),array_reverse($GLOBALS['search']), strip_tags($search))) . "%'";
                                }
                                
                                $query_video = mysqli_query($connect, "SELECT * FROM `tbl_video` ".$where_search." order by timestamp desc limit 0, 30");
                                $jumlah_video = mysqli_num_rows($query_video);
                                if($jumlah_video > 0){
                                    $limit_video_row = 3;
                                    $page_video_row_ = ceil($jumlah_video / $limit_video_row);
                                    $rest_video_row_ = $jumlah_video - (($page_video_row_ - 1) * $limit_video_row);
                                    $rest_hold = 0;
                                    $data_count = 0;
                                    while($hasil_video = mysqli_fetch_array($query_video)){
                                        if($jumlah_video - $data_count == $rest_video_row_){
                                            $rest_hold = 1;
                                        }
                                        if($limit_video_row == 4){
                                            $class_ = "col-md-3 col-sm-6 ";
                                        }
                                        if($limit_video_row == 3){
                                        ...
                                        $nama_kategori = "Video";
                                        $timestamp_ = $hasil_video['timestamp'];
                                        ...
                                        $query_kategori = mysqli_query($connect, "select judul_kategori from tbl_video_kategori where id = '".$hasil_video['id_kategori']."'");
                                        ...
                                        <div class="<?php echo $class_; ?> col-xs-12">
                                            <a href="video-<?php echo str_replace($GLOBALS['search'],$GLOBALS['replace'], strtolower($hasil_video['judul_video'])); ?>">
                                                <div class="card" style="min-height: 400px;">
                                                    ...
                                                    <div class="card-body">
                                                        ...
                                                        <div class="desc-article">
                                                            <span
                                                                class="d-block mb-2">
                                                                <strong style="display: -webkit-box; -webkit-line-clamp: 1; -webkit-box-orient: vertical; overflow: hidden;"><?php echo $hasil_video['judul_video']; ?></strong>
                                                            </span>
                                                            <span class="text-secondary small"><?php echo $all_day; ?></span>
                                                            <div class="limit-text-dashboard">
                                                                <?php echo substr(strip_tags($hasil_video['deskripsi_singkat']), 0, 200); ?>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
